...
import (
	...
	"github.com/xm-chentl/go-resource/dbfactory/errs"
	"github.com/xm-chentl/go-resource/postgres/grammar"
	"github.com/xm-chentl/go-resource/postgres/metadata"
...
func (q *query) Count(entry dbfactory.IDbModel) (res int64, err error) {
	defer q.reset()
...
func (q *query) Fields(fields ...interface{}) dbfactory.IQuery {
	if len(fields) > 0 {
...
func (q query) Find(res interface{}, opts ...dbfactory.IFindOptions) (err error) {
	resRt := reflect.TypeOf(res)
	resRv := reflect.ValueOf(res)
	if resRt.Kind() == reflect.Ptr {
		if resRt.Elem().Kind() == reflect.Slice {
...
func (q *query) Page(page int) dbfactory.IQuery {
	q.page = page
	if q.page < 1 {
		q.page = 1
	}
...
func (q *query) PageSize(pageSize int) dbfactory.IQuery {
	q.pageSize = pageSize
	if q.pageSize < 1 {
		q.pageSize = 20
	}
...
func (q *query) Asc(fields ...string) dbfactory.IQuery {
	for _, field := range fields {
...
func (q *query) Desc(fields ...string) dbfactory.IQuery {
	for _, field := range fields {
...
func (q *query) queryData(rt reflect.Type, resultsOfRv reflect.Value) (err error) {
	...
	sql, args := grammar.Select(
		metadata.Get(
			reflect.New(rt).Interface().(dbfactory.IDbModel),
		),
		q.fields,
		q.getArgs()...,
	)
