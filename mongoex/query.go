...
import (
	...
	"github.com/xm-chentl/go-resource/dbfactory/tools"
...
func (q *query) Fields(args ...interface{}) dbfactory.IQuery {
	if len(args) > 0 {
		q.projection = args[0]
	}
...
func (q *query) Find(res interface{}, opts ...dbfactory.IFindOptions) (err error) {
	...
	if collectionOptions != nil {
		...
		newEntry := reflect.New(resRt).Interface().(dbfactory.IDbModel)
...
func (q *query) First(res interface{}) (err error) {
	...
	entry, ok := res.(dbfactory.IDbModel)
	if !ok {
